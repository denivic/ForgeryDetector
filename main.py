...
def main():
    ...
    # Original images
    imgx, imgx_name = utils.load_image(r'C:\Users\Dumbledorf\Dropbox\Universitet\8. Semester (2022)\Bachelorprojekt\Kode\Data\Chunks\strip1._chunk1.png')
    imgy, imgy_name = utils.load_image(r'C:\Users\Dumbledorf\Dropbox\Universitet\8. Semester (2022)\Bachelorprojekt\Kode\Data\Chunks\strip1._chunk2.png')
    ...
    # Test 3: Compare SSIM and SSD
    ssimx = ssim(cv2.cvtColor(imgx, cv2.COLOR_BGR2GRAY), cv2.cvtColor(imgx, cv2.COLOR_BGR2GRAY))
    ssimy = ssim(cv2.cvtColor(imgx, cv2.COLOR_BGR2GRAY), cv2.cvtColor(imgy, cv2.COLOR_BGR2GRAY))
    ssdx = np.sum((imgx[:, :, 0:3] - imgx[:, :, 0:3])**2)
    ssdy = np.sum((imgx[:, :, 0:3] - imgy[:, :, 0:3])**2)
    print(f'SSIM for comparison of the same image ({imgx_name}): {ssimx}\nSSD for comparison of the same image ({imgx_name}): {ssdx}')
    print(f'SSIM for comparison of two different images ({imgx_name}, {imgy_name}): {ssimy}\nSSD for comparison of two different images({imgx_name}, {imgy_name}): {ssdy}')
